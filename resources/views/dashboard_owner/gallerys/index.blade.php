...
@section('title', __('owner.interior'))
    ...
    <div class="page-header">
        ...
        <nav class="breadcrumb-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><span>@lang('owner.the_banners')</span></li>
                <li class="breadcrumb-item active" aria-current="page"><span>@lang('owner.interior')</span></li>
            </ol>
        </nav>{{-- breadcrumb --}}
    </div>{{-- page-header --}}
            ...
            <div class="widget-content widget-content-area br-6">
            	<div class="table-form">
                    <div class="form-group row">
                        ...
                        <div class="col-4">
                            <a href="{{ route('dashboard.owner.gallerys.create') }}" class="btn btn-primary">@lang('dashboard.create')</a>
                        </div>
                    </div>
                </div>{{-- table-form --}}
            </div>{{-- widget-content --}}
                    ...
                    <tbody>
                    	@foreach ($gallerys as $index=>$gallery)
	                        ...
	                        <tr>
	                            <td>{{ $index+1 }}</td>
	                            <td>{{ $gallery->title }}</td>
	                            <td><img src="{{ $gallery->image_path }}" width="350"></td>
	                            <td>{{ $gallery->created_at->toFormattedDateString() }}</td>
	                            <td>
                                    {{-- <a href="{{ route('dashboard.owner.gallerys.edit',$gallery->id) }}" data-toggle="tooltip" data-placement="top" title="Edit">
                                    ...
                                    <form method="post" action="{{ route('dashboard.owner.gallerys.destroy', $gallery->id) }}" style="display: inline-block;">
                                        @csrf
                                        @method('delete')
